...
MONGO_URI = 'mongodb+srv://godb.net/?retryWrites=true&w=majority&appName=Cluster1'
client = MongoClient(MONGO_URI)
db = client['Harvinder']
users_collection = db['newPAID']
settings_collection = db['settings0']  # A new collection to store global settings
redeem_codes_collection = db['redeem_codesPaid']

# Bot Configuration
TELEGRAM_BOT_TOKEN = '7856318383:AAGSMi-PmQhOOAPYPzj7eWg5pyYys9-V2o4'
...
async def start(update: Update, context: CallbackContext):
    ...
    if not await is_user_allowed(user_id):
        await context.bot.send_message(chat_id=chat_id, text="*❌ You are not authorized to use this bot! Get access from @VirtualVenture.*", parse_mode='Markdown')
        return
